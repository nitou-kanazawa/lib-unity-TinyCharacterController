...
namespace Nitou.Pool
{
    ...
    [AddComponentMenu("Nitou/Pool/" + nameof(GameObjectPool))]
                                  ...
                                  IEquatable<GameObjectPool>
    {
        ...
        [DisableInPlayMode]
        [SerializeField]
        private bool _isActiveOnGet = true;
        ...
        private bool _hideSpawnObject;
#endif
        
#if UNITY_EDITOR
        [ShowInInspector, ReadOnly]
        private int ActiveObjectCount => _createdObjects.Count - _free.Count;

        [ShowInInspector, ReadOnly]
        private int CachedObjectCount => _free.Count;

        [ShowInInspector, ReadOnly]
        private int TotalObjectCount => _createdObjects.Count;

        [Button("Force Release All")]
        private void EditorReleaseAll() => ReleaseAllInstance();

        [Button("Destroy Unused")]
        private void EditorDestroyUnused() => DestroyUnusedObjects();
#endif
        ...
        /// <summary>
        /// Stores instances that were created with <see cref="Get(PooledGameObject)"/> and subsequently released with <see cref="Release(IPooledObject)"/>.
        /// When you call <see cref="Get(PooledGameObject)"/> again, the elements are retrieved from this list.
        /// NOTE: Use the component, not the interface, to know that the component has been destroyed from GameObject deletion.
        /// </summary>
        ...
        {
            while (_free.TryPop(out var instance)) 
        ...
        {
            // Prefabのnullチェック
            Assert.IsNotNull(_prefab, $"{nameof(_prefab)} is null.");
            ...
            Assert.IsFalse(
                _prefab.gameObject.activeSelf && _isActiveOnGet == false,
